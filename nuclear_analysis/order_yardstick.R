...
divergence_clean <- divergence_clean %>%
  filter(!is.na(query_alignment_percent), !is.na(ref_alignment_percent),
...
cat("\n=== ORDER R² DISTRIBUTION (cutoff justification) ===\n")
cat(sprintf("Orders with an R²: %d | median %.3f | mean %.3f\n",
            length(r2_vec), median(r2_vec), mean(r2_vec)))
print(r2_quantile_table)
cat(sprintf("Fixed R² = %.2f is the %.1fth percentile; cutoff used = %.3f (mode: %s)\n",
            R2_THRESHOLD, r2_threshold_pctile, r2_cutoff_used, R2_CUTOFF_MODE))
...
# class lmm with order random intercept (tier-b fallback)
...
# (a) class yardsticks fit on the same genus-capped data
CLASS_YARDSTICK_CSV <- ""  # optional path to class_yardstick_table.csv
...
# (c) score each pair vs its order (tier a), else its class
scored_all <- yardstick_data %>%
  ...
  filter(!is.na(ref_slope)) %>%  # drops pairs with no reference (e.g. Amphibia)
  mutate(
...
# per-order summary (order-scored pairs)
...
cat("\nDone. Order yardstick written to: ", normalizePath(OUT_DIR), "\n")