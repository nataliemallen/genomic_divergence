...
diagnose_class <- function(fit, df_class, class_name) {
  ...
  # slopes above/below breakpoint
  seg_slopes <- if (!is.null(seg)) {
    tryCatch(slope(seg)$log_div_time, error = function(e) NULL)
  } else NULL
  ...
  n_below <- if (!is.na(seg_breakpoint_mya))
    sum(df_class$timetree_div <  seg_breakpoint_mya, na.rm = TRUE) else NA_integer_
  ...
  list(
    ...
    slope_below = slope_below, slope_below_se = slope_below_se,
    slope_above = slope_above, slope_above_se = slope_above_se,
...
diagnostics_table <- map_dfr(
  ...
  ~ tibble(class = .x$class,
           ...
           slope_below = .x$slope_below, slope_below_se = .x$slope_below_se,
           slope_above = .x$slope_above, slope_above_se = .x$slope_above_se,
           n_below = .x$n_below, n_above = .x$n_above)
)
