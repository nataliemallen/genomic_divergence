...
setwd("/scratch/gautschi/allen715/GD_models/rate_model")  # same working dir as the PMM
...
PPCA_TRAIT_SPECS <- list(
  ...
  log_genome_size = list(col = "Genome.size",             fun = function(x) log(x)),
  gc              = list(col = "Genome.GC.content",       fun = function(x) x),
  log_n50         = list(col = "N50_bp",                  fun = function(x) log(x))
)
...
compute_ppca <- function(tree, class_name,
  ...
  if (nrow(mat) < 10) {
    cat(sprintf("too few complete species\n",
  ...
  for (i in seq_along(ax_names)) {
    ...
    ok <- if (is.na(p)) "? (Moran failed)" else
      ...
      else "** reconsider n global/local axes **"
...
fit_ppca_regression <- function(df, class_name, model_type) {
  ...
  list(fit = lm(as.formula(formula_str), data = df_complete), df = df_complete)
}
...
coef_path <- function(cn, tid, mt) {
  sprintf("results/ppca/coef_%s_%s_tree%04d.rds", cn, mt, tid)
}
...
for (cn in CLASSES) {
  ...
  for (tid in tree_ids) {
    ...
    done <- all(file.exists(coef_path(cn, tid, "ppca_avg")),
                file.exists(coef_path(cn, tid, "ppca_diff")))
    ...
    for (mt in c("ppca_avg", "ppca_diff")) {
      cp <- coef_path(cn, tid, mt)
      if (file.exists(cp)) { cat(sprintf("  %s tree %d already done\n", mt, tid)); next }
      res <- tryCatch(fit_ppca_regression(df_ppca, cn, mt),
                      error = function(e) { cat(sprintf("  %s failed: %s\n", mt, e$message)); NULL })
      if (is.null(res)) next
      saveRDS(tidy_lm(res$fit) %>%
                dplyr::mutate(class = cn, model_type = mt, tree_id = tid), cp)
      # residual Moran diagnostic 
      pm <- resid_moran(res$fit, res$df, tree)
...
pool_axis_effects <- function(cn, mt) {
  ...
  dplyr::bind_rows(lapply(files, readRDS)) %>%
    ...
    dplyr::summarize(m        = dplyr::n(),
                     Qbar     = mean(estimate),
                     Ubar     = mean(se^2),
                     B        = if (dplyr::n() > 1) var(estimate) else 0,
                     frac_sig = mean(p < 0.05),
                     .groups  = "drop") %>%
    dplyr::mutate(pooled_se   = sqrt(Ubar + (1 + 1 / m) * B),
                  estimate    = Qbar,
                  ci_lo       = Qbar - 1.96 * pooled_se,
                  ci_hi       = Qbar + 1.96 * pooled_se,
                  significant = sign(ci_lo) == sign(ci_hi),
                  class = cn, model_type = mt, n_trees = m,
                  predictor = unname(lab[term])) %>%
    dplyr::filter(!is.na(predictor)) %>%
    dplyr::select(class, model_type, predictor, estimate, pooled_se,
                  ci_lo, ci_hi, significant, frac_sig, n_trees)
}
...
if (nrow(axis_effects) > 0) {
  axis_effects %>%
    dplyr::arrange(class, model_type, dplyr::desc(abs(estimate))) %>%
    print(n = 100)
  write_csv(axis_effects, "results/ppca/ppca_axis_effects.csv")
} else {
  cat("\nNo pPCA coefficient files found to summarize.\n")
}
