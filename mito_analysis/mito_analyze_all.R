...
# read an .rds, returning NULL if missing or corrupt
read_rds_safe <- function(path) {
  if (!file.exists(path)) { warning(sprintf("Missing rds: %s", path)); return(NULL) }
  tryCatch(readRDS(path),
           error = function(e) {
             warning(sprintf("Skipping unreadable rds (%s): %s",
                             conditionMessage(e), path)); NULL })
}
...
load_and_pool <- function(class_name, model_type, variant = MODEL_VARIANT) {
  ...
  pooled <- map_dfr(tree_ids, function(tid) {
    ...
    post <- read_rds_safe(path); if (is.null(post)) return(NULL)
    post$tree_id <- tid
    post
  })
...
convergence_all <- map_dfr(CLASSES, function(cn) {
  map_dfr(discover_trees(cn, "avg_only"), function(tid) {
    ...
    s <- read_rds_safe(path); if (is.null(s)) return(NULL)
...
vc_all <- map_dfr(CLASSES, function(cn) {
  map_dfr(discover_trees(cn, "avg_only"), function(tid) {
    ...
    extract_vc <- function(vc, model_label) {
      # genus is not a random effect; phylogeny captures genus-level structure
      tryCatch({
        tibble(
          class       = cn,
          tree_id     = tid,
...
plot_raw_relationships <- function(df, class_name, model_type,
  ...
  pretty <- c(
    ...
    z_pop_density_avg = "Population density (avg)", z_mitogc_avg = "Mito GC (avg)",
    z_body_size_diff = "Body size (diff)", z_gen_time_diff = "Generation time (diff)",
    z_clutch_diff = "Clutch size (diff)", z_mitogc_diff = "Mito GC (diff)"
  )
...
find_scaled_df <- function(cn) {
  cands <- c(
    ...
    sprintf("/scratch/gautschi/allen715/GD_mito/results/models/%s_scaled_df.rds", cn)
    # nuclear fallback removed so mito never reads nuclear scaled_df
  )
...
for (cn in CLASSES) {
  ...
  df_scaled <- read_rds_safe(dfp); if (is.null(df_scaled)) next
