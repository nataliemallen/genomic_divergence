# dyadic phylogenetic mixed model pipeline
# using r/4.5.2
# libraries
library(tidyverse)
library(ape)
...
library(callr)
### parameters
RUN_CLASS <- "birds" # or "mammals"
N_TREES      <- 100          # tree posterior sample size (final paper: 100)
SEED         <- 12345
RUN_PHYLO_SIGNAL_TESTS  <- TRUE   # pagel's λ and Blomberg's K (tree 1 only)
RUN_SENSITIVITY_ANALYSES <- TRUE  # random-effects structure comparison (standard model only)
# model variant
# "standard" : log(k2p) ~ predictors + (1|mm(...))
# "rate"      : log(k2p/div_time) ~ predictors (no div_time predictor)
MODEL_VARIANT <- "standard"  # change to "rate" for rate model
# MCMC parameters
N_CHAINS      <- 4      # independent chains
N_ITER        <- 6000   # total iterations per chain
N_WARMUP      <- 2000   # warmup/burn-in
...
# NOTE: the phylogenetic PCA is now a SEPARATE script (ppca_analysis.R) that runs
# on the same trees this script saves to results/tree_objects/. See that script.
### output directories
...
dir.create("results/checkpoints",              showWarnings = FALSE)  # needed by checkpoint system
dir.create("results/downsampling_sensitivity", showWarnings = FALSE)  # needed by tree-1 save
dir.create("results/tree_objects",             showWarnings = FALSE)  # per-tree pruned trees (for pPCA)
### logging
...
cat("=================================================\n\n")
# 1) load data
setwd("/scratch/gautschi/allen715/GD_models/rate_model")
...
cat(sprintf("  Removed due to missing alignment data: %d\n",
            sum(is.na(divergence$query_alignment_percent) &
                  !divergence$genome1 %in% excluded_species)))
...
# Range overlap enters the models as a BINARY presence/absence term, `sympatric`
# (1 if ranges overlap at all). A continuous degree-of-overlap term was tested
# and dropped: the overlap magnitude among sympatric pairs is strongly bimodal
# (most pairs either barely overlap or are fully range-nested), so a linear
# gradient is a poor fit; it also costs ~15% of pairs (needs both range areas)
# and was non-credible. Binary sympatry is the clean geographic control.
...
cat(sprintf(
  "Sympatric pairs: %d | Allopatric pairs: %d | Missing overlap data: %d\n",
  sum(merged_data$sympatric == 1, na.rm = TRUE),
  sum(merged_data$sympatric == 0, na.rm = TRUE),
  sum(is.na(merged_data$sympatric))
))
...
cat(sprintf("Falco pairs before: %d\n",
            sum(analysis_data$genus_final           == "falco" & analysis_data$Class_final           == "Aves")))
cat(sprintf("Falco pairs after:  %d (should be <= %d)\n",
            sum(analysis_data_downsampled$genus_final == "falco" & analysis_data_downsampled$Class_final == "Aves"),
            MAX_PAIRS_PER_GENUS))
...
for (class_name in names(data_by_class)) {
  ...
  # complete cases include sympatric since it is a model predictor
  n_complete <- df %>%
    filter(!is.na(Body.size..kg._avg),         !is.na(Generation.time..years._avg),
           ...
           !is.na(N50_bp_avg),                  !is.na(sympatric)) %>%
    nrow()
...
# 14) log and z-score transform predictors
scale_predictors <- function(df) {
  df %>%
    mutate(
      ...
      # Averages: log of the pairwise mean.
      # Differences: log-ratio |log(x1) - log(x2)| to account for proportional dissimilarity
      ...
      log_genome_size_diff = abs(log(Genome.size_sp1) - log(Genome.size_sp2)),
      log_n50_avg          = log(N50_bp_avg),
      log_n50_diff         = abs(log(N50_bp_sp1) - log(N50_bp_sp2)),
...
# 16) phylogenetic signal tests - Pagel's λ and Blomberg's K
...
get_model_formula <- function(class_name, model_type = "avg_only",
  ...
  if (model_variant == "rate") {
    ...
    random_effects <- "(1 | mm(sp1, sp2, cov = A))"
    # genus removed: phylogenetic RE already captures genus-level structure
  }
...
# 20) sensitivity analysis - tests whether genus/family includes fit
fit_sensitivity_models <- function(df, A, class_name) {
  if (MODEL_VARIANT != "standard") {
    cat(sprintf(
      "  Skipping sensitivity analysis for %s (variant = '%s' — requires 'standard')\n",
  ...
  cat("\n1. Phylogeny-only model (matches main model)...\n")
  fit_base   <- fit_with_args(formula_base)
  cat("\n2. Model adding genus effect...\n")
  fit_genus  <- fit_with_args(formula_with_genus)
  cat("\n3. Model adding family effect...\n")
...
# 21) load tree selection utilities ───────────────────────────────────────────
...
run_single_tree_analysis <- function(tree_id, tree_config, data_by_class) {
  ...
  for (class_name in class_names) {
    ...
    # Resume: the PMM (avg + diff) is checkpointed as a unit per class/tree.
    ...
    # Save the exact pruned tree so ppca_analysis.R runs on IDENTICAL trees.
    saveRDS(tree_pruned,
...
# 25) run models
cat("\n=== SCALING PREDICTORS (full dataset, before tree loop) ===\n")
data_by_class <- lapply(data_by_class, scale_predictors)
cat("  Log transforms and z-scores applied.\n\n")
set.seed(SEED)
...
### calculate VIF for these models
